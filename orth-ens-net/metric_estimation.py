...
def get_ensemble_variance(case_folder, subject_id, mfiles=None, segment=False):
    ensemble_mean_prob = get_wmh_probabilities(case_folder, ensemble=True,
                                               mfiles=mfiles)
    if segment == True:
        ensemble_lm = ensemble_label_map = (ensemble_mean_prob > 0.5).astype(
            float)
...
def get_wmh_probabilities(case_folder, ensemble=False, mfiles=None):
    if ensemble:
        probability_file = os.path.join(case_folder + str(0),
                                        'wmh_prediction.nii.gz')
        img_shape = (nib.load(probability_file).get_fdata()).shape
        probability_prediction = np.zeros(
...
def ensemble_segmentacion(case_folder, M):
    ensemble_probs = get_wmh_probabilities(case_folder, ensemble=True,
                                           mfiles=M)
...
def brier_score(case_folder, subject_id, ensemble=False, mfiles=None):
    ...
    probabilities = get_wmh_probabilities(case_folder, ensemble=ensemble,
                                          mfiles=mfiles).flatten()
...
def save_ensemble_pred(case_folder, model_fold, subject_id, mfiles):
    ...
    nib.save(nib.Nifti1Image(probs, None),
             "./ensemble_preds/{}/{}/ensemble_wmh.nii.gz".format(subject_id,
                                                                 model_fold))
    nib.save(nib.Nifti1Image((probs > 0.5).astype(float), None),
             "./ensemble_preds/{}/{}/wmh_mask.nii.gz".format(subject_id,
                                                             model_fold))
        ...
        for k in range(kcross):
            ...
            for base_case_folder in glob(folder + '/*'):
                ...
                if metric == 'dice':
                    hard_dice = [dice_coefficient(
                        load_segmentation(gt_directory, subject_id, True),
                        ensemble_segmentacion(case_folder,
                                              sub_models_numbers))]
                    rows.append(hard_dice)
                elif metric == 'brier_plus':
                    brier = [brier_plus(case_folder, subject_id, ensemble=True,
                                        mfiles=sub_models_numbers)]
                    print('Brier Score: ', brier)
                    rows.append(brier)
                elif metric == 'brier':
                    brier = [
                        brier_score(case_folder, subject_id, ensemble=True,
                                    mfiles=sub_models_numbers)]
                    print('Brier Score: ', brier)
                    rows.append(brier)
                elif metric == 'variance':
                    var = [get_ensemble_variance(case_folder,
                                                 mfiles=sub_models_numbers)]
                    print('Variance: ', var)
                    rows.append(var)
...
if __name__ == "__main__":
    ...
    for model_fold in model_folds:
        testing_models(metrics, model_fold, n_models, folder,
                       save_results_fold)
        for Nnet in Nnets:
            testing_ensemble(int(Nnet), kcross, metrics, model_fold, n_models,
