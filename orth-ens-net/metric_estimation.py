...
from utils import set2to0, ensure_dir, or_img_labels
...
def brier_plus(**kwargs):
    ...
    fg_gt = gt[gt == 1]
    fg_prob = prob[gt == 1]
    return brier_score_loss(fg_gt, fg_prob) if len(fg_prob) > 0 else 'NaN'
...
def save_ensemble_pred(prob_image, mask_image, gt_image, k, ensemble_size,
    ...
    preds_folder = os.path.join(root_folder, 'images', ens_name, subject_id)
...
def load_image(path: str, labels: list = None) -> np.ndarray:
    ...
    # If combine_labels exists in the .ini, use the combination
    if 'comb_labels' in globals() and comb_labels is not None:
        suffix = dataset.split('_')[1]  # brats_TC -> TC
        if suffix not in comb_labels:  # ET:1;TC:1,4;WT:1,2,4
            raise ValueError(f"{suffix} suffix not found in the list of valid"
                             " label combinations "
                             f"({', '.join(list(comb_labels))}).")
        out_img = or_img_labels(out_img, {suffix: comb_labels[suffix]},
                                one_img=True)
...
def test_models(metrics: list, model_fold: str, n_models: int,
    ...
    if not overwrite and len(os.listdir(csv_folder)):
        print(f"{csv_folder} already contains files. Run metric_estimation "
              f"with --overwrite for computing the metrics again.")
        return
...
def load_ensemble_images(ens_models_paths: list, dataset: str, flatten=True,
    ...
    mask_img = np.array(np.mean(prob_imgs, axis=0) > 0.5, dtype=prob_img.dtype)
...
def test_ensembles(ensemble_size: int, k_cross: int, metrics: str,
                   ...
                   out_metrics_folder: str, dataset: str):
    """
        ...
        for metric in metrics:
            ...
            for pred_subject in glob(predictions_folder + '/*'):
                ...
                mask_image, prob_image, inp_size = load_ensemble_images(
                    ens_models_paths, dataset, flatten=True, get_size=True)
                gt_image = load_image(gt_path, labels, ).flatten()
...
if __name__ == "__main__":
    ...
    for fold in model_folds:  # XX_ResUNet_inter-orthogonal_selfp_Y_interp_Z
        # If there are combined labels, run for each combination (as a "new
        # dataset") as binary. Example: brats_ET, brats_TC ...
        datasets = [f'{dataset}_{e}' for e in
                    list(comb_labels.keys())] if comb_labels else [dataset]
        for dataset in datasets:
            test_models(metrics, fold, n_models, preds_folder,
                        out_metrics_folder, dataset)
