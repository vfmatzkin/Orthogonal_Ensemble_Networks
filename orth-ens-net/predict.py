...
def load_and_predict_raw_image(hold_out_images, model_name, model_fold,
    ...
    for filename in hold_out_images:
        ...
        save_folder = os.path.join(segmentation_directory,
                                   os.path.basename(filename), model_fold,
                                   model_name)
        print('filename: ', save_folder)
        flair_nii_gz = load_nii(
            os.path.join(origin_directory, filename, 'pre', 'FLAIR.nii.gz'))
        header_info = flair_nii_gz.header
        flair = np.array(flair_nii_gz.get_fdata())
        t1 = load_nii(os.path.join(origin_directory, filename, 'pre',
                                   'T1.nii.gz')).get_data()
        ...
        new_shape_all_modalities_joined = (
            2, t1.shape[0], t1.shape[1], t1.shape[2])
        all_modalities_joined = np.empty(new_shape_all_modalities_joined)
        all_modalities_joined[0] = t1
        all_modalities_joined[1] = flair

        all_modalities_joined = np.swapaxes(
            np.swapaxes(np.swapaxes(all_modalities_joined, 0, 1), 1, 2), 2, 3)
        ...
        if depth_with_padding_x != None:
            y_pred = y_pred[:, np.floor_divide(
                (depth_with_padding_x - original_shape[0]), 2):-math.ceil(
                (depth_with_padding_x - original_shape[0]) / 2), :, :, :]

        if depth_with_padding_y != None:
            y_pred = y_pred[:, :, np.floor_divide(
                (depth_with_padding_y - original_shape[1]), 2):-math.ceil(
                (depth_with_padding_y - original_shape[1]) / 2), :, :]

        if depth_with_padding_z != None:
            y_pred = y_pred[:, :, :, np.floor_divide(
                (depth_with_padding_z - original_shape[2]), 2):-math.ceil(
                (depth_with_padding_z - original_shape[2]) / 2), :]

        output_wmh = y_pred.astype(float)[0]
        ensure_dir(save_folder)
        nib.save(nib.Nifti1Image(output_wmh, None, header_info),
                 "{}/wmh_prediction.nii.gz".format(save_folder))
        nib.save(nib.Nifti1Image((output_wmh > 0.5).astype(float), None,
                                 header_info),
                 "{}/wmh_mask.nii.gz".format(save_folder))
